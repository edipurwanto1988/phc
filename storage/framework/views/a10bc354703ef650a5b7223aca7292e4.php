...
<div class="flex items-center justify-between w-full">
    <div class="flex items-center gap-2">
        ...
        <span>Edit Order: <?php echo e($order->order_number); ?></span>
    </div>
    <a href="<?php echo e(route('admin.orders.show', $order)); ?>" class="btn border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold px-4 py-2 rounded-lg text-sm transition-all">
        Batal
    </a>
</div>
        ...
        <form method="POST" action="<?php echo e(route('admin.orders.update', $order)); ?>">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-600 text-sm" value="<?php echo e($order->customer->nama); ?> (<?php echo e($order->customer->no_wa); ?>)" disabled>
                </div>

                <!-- Waktu Pengerjaan -->
                <div>
                    <label for="tanggal_jadwal" class="block text-sm font-semibold text-gray-700 mb-1">Jadwal Pengerjaan</label>
                    <input type="datetime-local" name="tanggal_jadwal" id="tanggal_jadwal" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" value="<?php echo e($order->tanggal_jadwal->format('Y-m-d\TH:i')); ?>" required>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="alamat_pengerjaan" id="alamat_pengerjaan" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" required><?php echo e($order->alamat_pengerjaan); ?></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="latitude" id="latitude" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" value="<?php echo e($order->latitude); ?>">
                </div>
                <div>
                    <label for="longitude" class="block text-sm font-semibold text-gray-700 mb-1">Longitude</label>
                    <input type="text" name="longitude" id="longitude" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" value="<?php echo e($order->longitude); ?>">
                </div>
            </div>
                    ...
                    <div class="p-4 border border-gray-200 rounded-lg bg-gray-50 flex flex-col md:flex-row gap-4 items-end">
                        ...
                        <div class="w-full md:flex-1">
                            ...
                            <select :name="'items['+index+'][service_id]'" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-white" x-model="item.service_id" @change="updateItemPrice(index)" required>
                                ...
                                <option value="<?php echo e($service->id); ?>" data-harga="<?php echo e($service->harga); ?>" data-satuan="<?php echo e($service->satuan); ?>"><?php echo e(!empty($service->nama_invoice) ? $service->nama_invoice : $service->nama); ?> (Rp <?php echo e(number_format($service->harga, 0, ',', '.')); ?> / <?php echo e($service->satuan); ?>)</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select name="metode_bayar" id="metode_bayar" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-white" required>
                        <option value="cash" <?php echo e($order->metode_bayar === 'cash' ? 'selected' : ''); ?>>Tunai (Cash)</option>
                        <option value="transfer" <?php echo e($order->metode_bayar === 'transfer' ? 'selected' : ''); ?>>Transfer Bank</option>
                        <option value="e-wallet" <?php echo e($order->metode_bayar === 'e-wallet' ? 'selected' : ''); ?>>E-Wallet (OVO/Dana/Qris)</option>
                    </select>
                </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="catatan" id="catatan" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" placeholder="Catatan instruksi pengerjaan khusus untuk cleaner..."><?php echo e($order->catatan); ?></textarea>
            </div>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
                ...
                <a href="<?php echo e(route('admin.orders.show', $order)); ?>" class="btn border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium px-5 py-2.5 rounded-lg text-sm transition-all">
                    Batal
                </a>
            </div>
        </form>
...
function orderForm() {
    return {
        diskon: <?php echo e((int)$order->diskon); ?>,
        items: [
            <?php $__currentLoopData = $order->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            { 
                service_id: '<?php echo e($item->service_id); ?>', 
                qty: <?php echo e($item->qty); ?>, 
                harga: <?php echo e((int)$item->harga_satuan); ?>, 
                satuan: '<?php echo e($item->satuan); ?>' 
            },
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ],
        addItem() {
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/macbook/CascadeProjects/PHC/laravel/resources/views/admin/orders/edit.blade.php ENDPATH**/ ?>