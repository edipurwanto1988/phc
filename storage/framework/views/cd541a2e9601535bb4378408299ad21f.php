...
<div class="card mb-6 bg-white p-6 rounded-xl border border-gray-200">
    <form method="GET" action="<?php echo e(route('admin.orders.index')); ?>" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        <!-- Search -->
        <div>
            <label for="search" class="block text-xs font-semibold text-gray-500 uppercase mb-1">Cari Pesanan</label>
            <input type="text" name="search" id="search" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" placeholder="No. Order / Nama Customer..." value="<?php echo e(request('search')); ?>">
        </div>
        ...
        <div>
            ...
            <input type="date" name="date" id="date" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" value="<?php echo e(request('date')); ?>">
        </div>
        ...
        <div class="flex gap-2">
            ...
            <a href="<?php echo e(route('admin.orders.index')); ?>" class="btn border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-2 px-3 rounded-lg text-sm transition-all" title="Reset Filter">
...
<div class="card">
    <div class="p-6 border-b border-gray-200 flex justify-between items-center bg-white rounded-t-xl">
        ...
        <?php if(auth()->user()->hasPermission('manage_orders') || auth()->user()->hasPermission('create_orders')): ?>
        <a href="<?php echo e(route('admin.orders.create')); ?>" class="btn bg-blue-600 hover:bg-blue-700 text-white font-medium px-4 py-2 rounded-lg flex items-center gap-1.5 shadow-sm text-sm">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                    ...
                    <th class="py-3.5 px-6 text-sm font-semibold text-gray-600 text-right"><?php echo e(auth()->user()->role->name === 'Cleaner' ? 'Gaji Diterima' : 'Grand Total'); ?></th>
                    <th class="py-3.5 px-6 text-sm font-semibold text-gray-600 text-center">Status</th>
                    <?php if(auth()->user()->role->name !== 'Cleaner'): ?>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td class="py-4 px-6 text-sm font-bold text-blue-600">
                        <a href="<?php echo e(route('admin.orders.show', $order)); ?>" class="hover:underline"><?php echo e($order->order_number); ?></a>
                    </td>
                    <td class="py-4 px-6">
                        <div class="text-sm font-semibold text-gray-800"><?php echo e($order->customer->nama); ?></div>
                    ...
                    <td class="py-4 px-6 text-sm font-bold text-gray-850 text-right">
                        <?php if(auth()->user()->role->name === 'Cleaner'): ?>
                            ...
                            ?>
                            Rp <?php echo e(number_format($myAssign->gaji ?? 0, 0, ',', '.')); ?>

                            <div class="text-[9px] <?php echo e(($myAssign->status_gaji ?? '') === 'sudah_dibayar' ? 'text-green-600' : 'text-red-500'); ?> font-bold">
                                <?php echo e(($myAssign->status_gaji ?? '') === 'sudah_dibayar' ? 'Sudah' : 'Belum'); ?> Dibayar
                            </div>
                        <?php else: ?>
                    ...
                    <td class="py-4 px-6 text-center">
                        <div class="flex items-center justify-center gap-3">
                            <a href="<?php echo e(route('admin.orders.show', $order)); ?>" class="text-gray-500 hover:text-gray-750 transition-colors" title="Lihat Detail">
                                <i class="ri-eye-line text-lg"></i>
                            </a>
                            <?php if(auth()->user()->hasPermission('manage_orders') || auth()->user()->hasPermission('edit_orders')): ?>
                            <a href="<?php echo e(route('admin.orders.download-invoice', $order)); ?>" class="text-blue-600 hover:text-blue-800 transition-colors" title="Download Invoice">
                                <i class="ri-download-2-line text-lg"></i>
                            </a>
                            <?php endif; ?>
                            <?php if((auth()->user()->hasPermission('manage_orders') || auth()->user()->hasPermission('delete_orders')) && $order->status_bayar !== 'paid'): ?>
                            <form method="POST" action="<?php echo e(route('admin.orders.destroy', $order)); ?>" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data order ini?')">
                                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/macbook/CascadeProjects/PHC/laravel/resources/views/admin/orders/index.blade.php ENDPATH**/ ?>