...
@section('header')
<i class="ri-shield-user-line"></i> Edit Role & Hak Akses
@endsection
    ...
    <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="p-6">
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="name" class="input w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" placeholder="Contoh: Staff Operator" value="{{ $role->name }}" required>
            </div>
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    <div class="space-y-2.5 p-4 bg-white rounded-lg border border-gray-150 shadow-sm">
                        ...
                        <label class="flex items-center gap-2.5 text-sm cursor-pointer hover:bg-gray-50 p-1.5 rounded transition-colors">
                            <input type="checkbox" name="permissions[]" value="{{ $perm }}" class="rounded text-blue-600 focus:ring-blue-500 w-4 h-4" {{ $role->hasPermission($perm) ? 'checked' : '' }}>
